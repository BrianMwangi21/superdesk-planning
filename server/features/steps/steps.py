...
def get_local_end_of_day(context, day=None, timezone=None):
    ...
    return tz.localize(datetime.combine(day, time(23, 59, 59)), is_dst=None).astimezone(pytz.utc)
...
async def then_we_get_text_in_response_field(context, field):
    ...
    assert context.text.strip() in response_text, '"{}" not in "{}"'.format(context.text.strip(), response_text)
...
async def then_we_get_array_of_by(context, field, fid):
    ...
    for row in response[field]:
        ...
        assert_equal(
            json_match(context_data[row[fid]], row), True, msg=str(row) + "\n != \n" + str(context_data[row[fid]])
        )
...
async def then_coverage_has_current_date(context, index):
    ...
    assert (
        response_date_time.date() == get_local_end_of_day(context).date()
...
def then_store_versioned_file(context, path):
    path = apply_placeholders(context, path)
    assert os.path.isfile(path), "{} is not a file".format(path)
